...
namespace ThreeRingsSharp.DataHandlers.Expressions {
	public static class FloatExpressionHandler {
		...
		/// <summary>
		/// Given a <see cref="FloatExpression"/> and an optional frame number (if this expression is part of an animation), this will attempt to compute its value. 
		/// Most expression types are supported, but some are not.
		/// </summary>
		/// <param name="expression"></param>
		/// <param name="frameNumber"></param>
		/// <returns></returns>
		public static float Compute(this FloatExpression expression, int frameNumber = 0) {
			...
			} else if (expression is FloatExpression.Clock) {
				return frameNumber;
