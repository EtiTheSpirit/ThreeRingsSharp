...
namespace ThreeRingsSharp.XansData.IO.GLTF {
	...
	public class GLTFExporter : AbstractModelExporter {
		...
		private byte[] GetJSONBuffer() {
			...
			jsonStr.WriteASCIIToByteArray(jsonBuffer, 8);
			return jsonBuffer;
		}
		...
		/// <returns></returns>
		/// 
		private long WriteBinaryData(BinaryWriter stream, Model3D[] models) {
			long current = stream.BaseStream.Position;

			stream.Write(0);
			stream.Write(0x004E4942); // The text "BIN" starting with null
			...
			foreach (Model3D model in models) {
				...
				if (modelMesh!.HasBoneData) {
					...
					foreach (string? riggedBoneName in modelMesh.BoneNames) {
						...
						*/
						stream.Write(bone.InverseReferenceTransform);
					}
						...
						for (int mode = 0; mode < 3; mode++) {
							foreach (Keyframe keyframe in ordered) {
								...
								if (key != null) {
									...
									(translation, rotation, scale) = new Transform3D().GetAllComponents();
			...
			long written = stream.BaseStream.Position - current;
			long bytesToAdd = written % 4;
			for (int idx = 0; idx < bytesToAdd; idx++) {
				stream.Write((byte)0);
				written++;
			}

			int writtenOffset = (int)(written - 8);
			...
			stream.Write(writtenOffset);
			stream.Seek(0, SeekOrigin.End);
			JSONData.Buffers.Add(new GLTFBuffer {
				ByteLength = writtenOffset
			});
		...
		[Obsolete("This method wastes a lot of memory.")]
		...
		public override void Export(Model3D[] models, FileInfo toFile) {
			...
			long written = WriteBinaryData(new BinaryWriter(mstr), models);
			byte[] jsonBuffer = GetJSONBuffer();
			writer.Write(MAGIC_NUMBER);
			writer.Write(VERSION);
			writer.Write(jsonBuffer.Length + (int)written + 12);
			writer.Write(jsonBuffer);

			mstr.Seek(0, SeekOrigin.Begin);
			mstr.CopyTo(writeStr);
		}
	}
}
